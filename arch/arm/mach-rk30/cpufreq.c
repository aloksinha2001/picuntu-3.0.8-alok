 ...
 *
 */
//OMEGAMOON_CHANGED define is used here only to avoid bizarre governor
//   behavior, see below, and to actually affect the policy's frequency!
...
//Galland: this is actually not a max freq, but the GPU freq from which to control temperature/frequency // Omegamoon >> Change gpu speed upper limit from 350 to 400
...
static bool rk30_cpufreq_is_ondemand_policy(struct cpufreq_policy *policy)
{
   ...
   */
   ...
   return expression, also interactive, conservative, h?? governors
...
static const struct cpufreq_frequency_table temp_limits_high[] = {
   {.frequency =  816 * 1000, .index = 100}, //Galland: index 100?? then GPU will "never" affect the max CPU freq in rk30_cpufreq_temp_limit_work_func below!
};
...
static void rk30_cpufreq_temp_limit_work_func(struct work_struct *work)
{
   ...
   temp = (rk30_tsadc_get_temp(0) > rk30_tsadc_get_temp(1) ? rk30_tsadc_get_temp(0) : rk30_tsadc_get_temp(1)); //Galland: there are 2 CPU cores
   FREQ_PRINTK_LOG("cpu_thermal(%d)\n", temp);

   gpu_irqs[1] = kstat_irqs(IRQ_GPU_GP);
   if (clk_get_rate(gpu_clk) > GPU_MAX_RATE) {
      delay = HZ / 20;
      if ((gpu_irqs[1] - gpu_irqs[0]) < 3) { //Galland: are these irqs supposed to mean something GPU-temp. wise?
