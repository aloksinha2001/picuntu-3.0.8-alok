...
int bt_enable() {
    ...
    // cmy@20120602: wait for BT work up
    usleep(BLUETOOT_POWER_USEC);
    ...
    LOGI("Waiting for HCI device present...");
    ...
    if (attempt == 0) {
        LOGE("%s: Timeout waiting for HCI device to come up, ret=%d",
            __FUNCTION__, ret);
...
int bt_is_powerup()
{
    return (check_bluetooth_power()>0)?1:0;
}
...
int bt_get_chipname(char* name, int len)
{
    ...
    if (fd < 0) {
        LOGE("open(%s) failed: %s (%d)", rfkill_name_path, strerror(errno),
             errno);
        goto out;
    }
    ...
    sz = read(fd, name, len);
    if (sz < 0) {
        LOGE("read(%s) failed: %s (%d)", rfkill_name_path, strerror(errno),
             errno);
        goto out;
    }
    name[sz] = '\0';
