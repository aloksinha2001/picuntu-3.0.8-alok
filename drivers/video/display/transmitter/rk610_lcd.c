...
#endif
static int rk610_lcd_scaler_bypass(struct i2c_client *client)
{
    RK610_DBG(&client->dev,"%s \n",__FUNCTION__);
...
int rk610_lcd_scaler_set_param(struct rk29fb_screen *screen,bool enable )//enable:0 bypass 1: scale
{
    ...
    RK610_DBG(&client->dev,"%s enable = %d\n",__FUNCTION__, enable);
    ...
    if(enable == 1){
        ...
        rk610_scaler_chg(client,screen);
	}
	else 
#endif
	{
	    ...
	    rk610_lcd_scaler_bypass(client);
	}
	return 0;
}
