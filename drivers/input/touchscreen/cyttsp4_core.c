...
#define CY_TOUCH_GPIO_RST_CYTTSP   RK30_PIN4_PD0
#define CY_TOUCH_GPIO_IRQ_CYTTSP   RK30_PIN4_PC2
...
static int _cyttsp4_hndshk_enable(struct cyttsp4 *ts)
{
	...
	tries = 0;
    while (tries++ < (10000/CY_DELAY_DFLT)) {
...
//#endif
			...
			if(e!=0x03)/* event code:0x01 down, 0x02 move, 0x03 up*/
			{
	...
	else //no data anymore, send end message for all finger touch 
	{
		for(i=0;i<10;i++)
		{			
...
int retval=0;
  ...
  //  while(1)
    {
	gpio_direction_input(CY_TOUCH_GPIO_IRQ_CYTTSP);
	gpio_set_value(CY_TOUCH_GPIO_IRQ_CYTTSP, 0);
	msleep(100);
	gpio_set_value(CY_TOUCH_GPIO_IRQ_CYTTSP, 0);
	msleep(2);
	gpio_direction_input(CY_TOUCH_GPIO_IRQ_CYTTSP);
...
// IC config CRC check
static int _cyttsp4_CRC_check(struct cyttsp4 *ts)
{
/*
	w 69 01 3B 00 00 4F 6D 17 ;;Exit Bootloader command
	W 67 00 08 ;Enter Operating mode
	
	W 67 03 00 ; Enter Block ID(default 0) in CMD_DATA[0] register
	W 67 02 05 ; Write “Get Config Block CRC” command code to command register
	W 67 02, r 67 X X X X ; Read Command Byte, status and CRC
*/
	int retval = 0;
	uint8_t BlockID  = 0x00;
	u8 cmd = 0;
	uint8_t response[4] = {0x00, 0x00, 0x00, 0x00};

	cmd = 0x00;
	retval = cyttsp4_write_block_data(ts, 0x03, 1, &cmd,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		pr_err("%s: Failed to CRC 1 r=%d\n",
			__func__, retval);
	}
	...
	cmd = 0x05;
	retval = cyttsp4_write_block_data(ts, 0x02, 1, &cmd,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		pr_err("%s: Failed to CRC 2 r=%d\n",
			__func__, retval);
	}
	...
	memset(response, 0, sizeof(response));
	retval = cyttsp4_read_block_data(ts,
		0x02, sizeof(response), &(response[0]),
		ts->platform_data->addr[CY_TCH_ADDR_OFS],
		true);
	if (retval < 0) {
		pr_err("%s: Failed to CRC 3 r=%d\n",
			__func__, retval);
	}
	
	//if((response[2]==0xA5)&&(response[3]==0xB5))  
	if((response[2]==0x72)&&(response[3]==0x95))
	{
		return 0;
	}
	else
...
//更新配置参数
static int  _cyttsp4_UpdateConfig(struct cyttsp4 *ts)
{
/*
	W 67 00 28 ;Configuration

	delay  70ms
	W 67 03 00 00 
	W 67 05 00 80 
	W 67 07 00
	W 67 08 00 5C 00 7E 00 00 14 1E 04 00 03 00 4D 95 3A 62
	W 67 18 00 0A 00 20 03 E8 00 00 00 00 23 02 00 02 00 02 
	W 67 28 00 0E 00 00 57 57 57 00 00 0C 09 00 00 64 0A 07
	W 67 38 D0 01 00 64 00 05 08 02 10 0A 04 00 13 88 00 C8
	W 67 48 00 32 00 00 03 E8 01 09 04 4D 95 3A 62 00 01 00 
	W 67 58 64 00 64 03 20 05 DC 20 00 80 00 78 00 00 00 00
	W 67 68 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
	W 67 78 00 00 00 00 00 00 00 00 00 00 00 00 00
	W 67 86 A5 B5
	w 67 88 45 63 36 6F 34 38 73 77
...
/*
	u8 config[] = {0x00, 0x00, 
				0x00, 0x80, 
				0x00,
				0x00, 0x5C, 0x00, 0x7E, 0x00, 0x00, 0x14, 0x1E, 0x04, 0x00, 0x03, 0x00, 0x4D, 0x95, 0x3A, 0x62,
				0x00, 0x0A, 0x00, 0x20, 0x03, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x23, 0x02, 0x00, 0x02, 0x00, 0x02, 
				0x00, 0x0E, 0x00, 0x00, 0x57, 0x57, 0x57, 0x00, 0x00, 0x0C, 0x09, 0x00, 0x00, 0x64, 0x0A, 0x07,
				0xD0, 0x01, 0x00, 0x64, 0x00, 0x05, 0x08, 0x02, 0x10, 0x0A, 0x04, 0x00, 0x13, 0x88, 0x00, 0xC8,
				0x00, 0x32, 0x00, 0x00, 0x03, 0xE8, 0x01, 0x09, 0x04, 0x4D, 0x95, 0x3A, 0x62, 0x00, 0x01, 0x00, 
				0x64, 0x00, 0x64, 0x03, 0x20, 0x05, 0xDC, 0x20, 0x00, 0x80, 0x00, 0x78, 0x00, 0x00, 0x00, 0x00,
	...
	u8 config[] = {0x00, 0x00, 
				...
				0x00,
				0x00, 0x5C, 0x00, 0x7E, 0x00, 0x00, 0x14, 0x1E, 0x04, 0x00, 0x03, 0x00, 0x4D, 0x95, 0x3A, 0x62,
				0x00, 0x0A, 0x00, 0x20, 0x00, 0x46, 0x00, 0x00, 0x00, 0x00, 0x23, 0x02, 0x00, 0x02, 0x00, 0x02, 
				0x00, 0x0E, 0x00, 0x00, 0x57, 0x57, 0x57, 0x00, 0x00, 0x0C, 0x09, 0x00, 0x00, 0x64, 0x0A, 0x07,
				0xD0, 0x01, 0x00, 0x64, 0x00, 0x05, 0x08, 0x02, 0x10, 0x0A, 0x04, 0x00, 0x13, 0x88, 0x00, 0xC8,
				0x00, 0x32, 0x00, 0x00, 0x03, 0xE8, 0x01, 0x09, 0x04, 0x4D, 0x95, 0x3A, 0x62, 0x00, 0x01, 0x00, 
				0x64, 0x00, 0x64, 0x03, 0x20, 0x05, 0xDC, 0x20, 0x00, 0x80, 0x00, 0x78, 0x00, 0x00, 0x00, 0x00,
	...
	cmd = CY_CONFIG_MODE + CY_MODE_CHANGE;
	retval = cyttsp4_write_block_data(ts, 0x00, 1, &cmd,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		pr_err("%s: Failed to UpdateConfig 1 r=%d\n",
			__func__, retval);
	}

	msleep(100);

	retval = cyttsp4_write_block_data(ts, 0x03, 141, &config,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		pr_err("%s: Failed to UpdateConfig 2 r=%d\n",
			__func__, retval);
	}
	...
	retval = cyttsp4_write_block_data(ts, 0x02, 1, &cmd,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		pr_err("%s: Failed to UpdateConfig 3 r=%d\n",
			__func__, retval);
	}
...
_cyttsp4_startup_wait_bl_irq:
    ...
    mdelay(500);
    buf[0] = 0x08;
    retval = cyttsp4_write_block_data(ts, 0x00, 1, &(buf[0]),
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
    mdelay(100);
    printk("%s: %d\n", __func__, retval);
	
	// CRC check
    if(_cyttsp4_CRC_check(ts) != 0) {
    //if (upflag) {
        // UpdateConfig
        mdelay(100);
        _cyttsp4_UpdateConfig(ts);
...
////////////////////////////////////////////////////////////////////
    ...
    input_mt_init_slots(ts->input, 255);
	...
	set_bit(ABS_MT_TOUCH_MAJOR, ts->input->absbit);
	set_bit(ABS_MT_WIDTH_MAJOR, ts->input->absbit);

	input_mt_init_slots(ts->input, 5);

	input_set_abs_params(ts->input,ABS_MT_POSITION_X, 0, 1024, 0, 0);
	input_set_abs_params(ts->input,ABS_MT_POSITION_Y, 0, 768, 0, 0);
	input_set_abs_params(ts->input,ABS_MT_TOUCH_MAJOR, 0, 15, 0, 0);
	input_set_abs_params(ts->input,ABS_MT_WIDTH_MAJOR, 0, 15, 0, 0);
